...
/*! *************************************************************************
 * Function Name : InodeDirValidateInodeisDir
 *
 * Author        : Xinyi Li
 *
 * Description   : This Function is used to check if a data block is a directory
 ...
 *
 **************************************************************************/
INT4 InodeDirValidateInodeisDir(UINT4 u4InodeNo){
    ...
    pBlockBuffer = Buffer;
    memset(&DirEntry, 0, sizeof(struct ext3_dir_entry_2));

    /*validate if the inode can be read or is used*/
    if (InodeUtilValidateIfInodeAvailable(u4InodeNo, &Inode) == INODE_FAILURE) {
        DEBUG_LOG("ERROR: cannot read this inode or this inode is unuesd");
        return INODE_FAILURE;
    }

    /*validate the dir block*/
    u4BlockNumber = Inode.i_block[FIRST_DIR_BLOCK_INDEX];
    ...
    i4RetVal = InodeDirValidateDataBlock(pBlockBuffer);
    if (i4RetVal == INODE_FAILURE) {
