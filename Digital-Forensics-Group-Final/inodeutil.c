...
//    i4RetVal = 0;
//
//    if (InodeUtilValidateIfInodeAvailable(u4InodeNo, &Inode) == INODE_FAILURE) {
//        DEBUG_LOG("ERROR: cannot read inode or this indoe is unused");
//        //todo: modify debug
...
//
///*! *************************************************************************
// * Function Name : InodeUtilPrintDirByInode
// *
// * Author        : Xinyi Li
// *
// * Description   : This Function is used to print the directory entries by
// *                 giving the directory's inode
// *
...
//INT4 InodeUtilPrintDirByInode(UINT4 u4InodeNo) {
//    struct htSortByCurDirInode *foundCurDir;
//    struct htFilesStoreInOneDir *foundFilesInCurDir;
//    struct htFilesStoreInOneDir *foundFilesTemp;
//
//    /*validate if this inode is a dir inode*/
//    if (InodeDirValidateInodeisDir(u4InodeNo) == INODE_FAILURE) {
//        DEBUG_LOG("ERROR: not a directory inode");
//        //todo: modify
//        return INODE_FAILURE;
//    }
//
...
//        /*if existingFiles is Null,scan frome root*/
//        if (InodeListScanFromThisDirInode(ROOT_INODE, "/") == INODE_FAILURE) {
//            DEBUG_LOG("ERROR: Failed to read this directory inode");
//            return INODE_FAILURE;
//        }
//    }
//    HASH_FIND_INT(existingFiles, &u4InodeNo, foundCurDir);
//    foundFilesInCurDir = foundCurDir->htFileStoreInThisDir;
//    for (foundFilesTemp = foundFilesInCurDir; foundFilesTemp != NULL; foundFilesTemp = foundFilesTemp->hh.next) {
//        struct ext3_dir_entry_2 DirEntry;
//        memset(&DirEntry, 0, sizeof(DirEntry));
//        DirEntry = foundFilesTemp->entryInfo->DirEntry;
...
INT4 InodeUtilReadDataBlock(UINT4 u4BlockNo, UINT2 u2StartPos, VOID *pBuffer, UINT4 u4Size) {
    ...
    if (lseek(gu4FileDes, u8Offset, SEEK_SET) == SYS_CALL_FAILURE) {
...
///*! *************************************************************************
// * Function Name : InodeUtilValidateIfInodeAvailable
// *
// * Author        : Xinyi Li
// *
// * Description   : This Function is used to validate if a inode is used or can be read
// *
// * Input         : u4InodeNo    -   Inode number to read
// *                 pInode       -   pointer to the structure containing the
// *                                  inode contents
// *
// * Output        : None
// *
// * Returns       : INODE_SUCCESS, this inode is available
// *                 INODE_FAILURE otherwise
// *
// **************************************************************************/
INT4 InodeUtilValidateIfInodeAvailable(UINT4 u4InodeNo, struct ext3_inode *pInode) {
    if (InodeUtilReadInode(u4InodeNo, pInode) == INODE_FAILURE) {
        DEBUG_LOG3("ERROR: Failed to read Inode: %d %s:%d\n", u4InodeNo, __FILE__,
                   __LINE__);
        return INODE_FAILURE;
    }
#if 1
    if (InodeUtilIsFreeInode(pInode) == TRUE) {
        printf(" Inode unused\n");
        return INODE_FAILURE;
    }
#endif
...
///*! *************************************************************************
// * Function Name : InodeUtilPrintAllDeletedFiles
// *
// * Author        : Xinyi Li
// *
// * Description   : This Function is used to print all deleted files
// *
// * Input         : Noe
// *
// * Output        : None
// *
// * Returns       : INODE_SUCCESS, if the inode is free
// *                 INODE_FAILURE otherwise
// *
// **************************************************************************/
//INT4 InodeUtilPrintAllDeletedFiles() {
//    struct htSortByCurDirInode *deletedFileIndex, *outIndex;
//    FILE *fDeletedFilesCSV;
//
//    if (tempDeletedFiles == NULL) {
//        /*scan from root*/
//        if (InodeListScanFromThisDirInode(ROOT_INODE, "/") == INODE_FAILURE) {
//            DEBUG_LOG("ERROR: Failed to read directory inode");
//            return INODE_FAILURE;
//        }
//    }
//
//    /*subtract existing files from tempDeletedFiles, and get a real deletedFiles*/
//    for (outIndex = tempDeletedFiles; outIndex != NULL; outIndex = outIndex->hh.next) {
//        struct htSortByCurDirInode *deletedFile;
//        struct htSortByCurDirInode *existTemp;
//        struct htFilesStoreInOneDir *deletedFileStoreInOneDir;
//        deletedFile = malloc(sizeof(struct htSortByCurDirInode));
//        UINT4 u4TempDeletedNo = outIndex->u4CurrentDirInodeNo;
//        deletedFileStoreInOneDir = NULL;
//        if (outIndex->htFileStoreInThisDir != NULL) {
//            HASH_FIND_INT(existingFiles, &u4TempDeletedNo, existTemp);
//            if (existTemp != NULL) {
//                struct htFilesStoreInOneDir *tempDeletedFile;
//                struct htFilesStoreInOneDir *existingFile;
//                struct htFilesStoreInOneDir *innerIndex, *temp;
//                tempDeletedFile = outIndex->htFileStoreInThisDir;
//                existingFile = existTemp->htFileStoreInThisDir;
//                HASH_ITER(hh, tempDeletedFile, innerIndex, temp) {
//                    UINT4 u4EntryInodeNo = innerIndex->u4InodeNo;
//                    struct htFilesStoreInOneDir *errorFile;
//                    HASH_FIND_INT(existingFile, &u4EntryInodeNo, errorFile);
//                    if (errorFile == NULL) {
//                        HASH_ADD_INT(deletedFileStoreInOneDir, u4InodeNo, innerIndex);
//                    }
//                }
//            } else {
//                deletedFileStoreInOneDir = outIndex->htFileStoreInThisDir;
//            }
//            if (deletedFileStoreInOneDir != NULL) {
//                deletedFile->u4CurrentDirInodeNo = u4TempDeletedNo;
//                deletedFile->htFileStoreInThisDir = deletedFileStoreInOneDir;
//                HASH_ADD_INT(deletedFiles, u4CurrentDirInodeNo, deletedFile);
//            }
//        }
//    }
//
//    fDeletedFilesCSV = fopen(DELETED_FILES_CSV_PATH, "w");
//    if (fDeletedFilesCSV == NULL) {
//        printf("ERROR: cannot open csv file\n");
//        return INODE_FAILURE;
//    }
//
//    printf("---------------------------------------------------------------------------------------------\n");
//    printf("|  Inode no.   |                name                 |                  path                |\n");
//    printf("---------------------------------------------------------------------------------------------\n");
//
//    /*print all deleted files, and output them into a csv file*/
//    for (deletedFileIndex = deletedFiles; deletedFileIndex != NULL; deletedFileIndex = deletedFileIndex->hh.next) {
//        struct htFilesStoreInOneDir *fileIndex;
//        struct htFilesStoreInOneDir *fileStoreInOneDirHashTable;
//
//        fileStoreInOneDirHashTable = deletedFileIndex->htFileStoreInThisDir;
//        if (fileStoreInOneDirHashTable != NULL) {
//            for (fileIndex = fileStoreInOneDirHashTable; fileIndex != NULL; fileIndex = fileIndex->hh.next) {
//                struct ext3_dir_entry_2 DirEntry = fileIndex->entryInfo->DirEntry;
//                char *path = fileIndex->entryInfo->path;
//                char nullName[EXT3_NAME_LEN];
//                memset(nullName,0, sizeof(nullName));
//                printf("| %12u | %35s |  %35s |\n", DirEntry.inode, DirEntry.name, path);
//                if (strcmp(DirEntry.name, nullName) == 0) {
//                    strncpy(DirEntry.name, "cannot read name", sizeof("cannot read name"));
//                }
//                if (fprintf(fDeletedFilesCSV, "%u,%u,%s,%s,%u\n", deletedFileIndex->u4CurrentDirInodeNo, DirEntry.inode,
//                            DirEntry.name, path,DirEntry.file_type) == EOF) {
//                    DEBUG_LOG2("ERROR: cannot print file whose inode number is %u, "
//                                       "which belong to directory whose inode number is %u into CSV file",
//                               deletedFileIndex->u4CurrentDirInodeNo, DirEntry.inode);
//                    return INODE_FAILURE;
//                }
//            }
//        }
//    }
//    if (fclose(fDeletedFilesCSV) == EOF) {
//        DEBUG_LOG("ERROR: cannot close csv file");
//        return INODE_FAILURE;
//    }
//
//    printf("---------------------------------------------------------------------------------------------\n");
